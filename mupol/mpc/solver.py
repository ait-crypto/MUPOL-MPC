...
class MPCSolver:
    ...
    async def _fill_truck_with_order(self, truck: Truck, order: Order) -> None:
        ...
        async with mpc:
            ...
            order.process_this_round = await real_or(
                compatible, order.process_this_round
            )
    ...
    async def _fill_trucks(self) -> None:
        ...
        for truck in self.trucks:
            ...
            truck.destination = mpc.input(
                self.secure_node_type(self.dummy_node), senders=mpc.pid
            )
    ...
    async def _find_distances_to_order(
        ...
        async with mpc:
            ...
            route_matrix = [
                [
                    mpc.input(self.secure_node_type(entry), senders=mpc.pid)
                    for entry in row
                ]
    ...
    async def _create_empty_truck_drive(self) -> None:
        ...
        async with mpc:
            ...
            freighter_id = await self._find_freighter_id(
                truck_index=closest_truck_index
            )
    ...
    async def solve_problem(self) -> None:
        """Solver function."""
        self.logger.debug(
            "Running solver for %s orders, %s trucks, % freighters",
            len(self.orders),
            len(self.trucks),
            len(self.freighters),
        )
        ...
        async with mpc:
            ...
            for order in self.orders:
                ...
                self.logger.debug(
                    "Destination: %s", await mpc.output(order.destination)
                )
